...
func newFuncInfo(parent *funcInfo, fd *FunctionDefineExpression) *funcInfo {
	return &funcInfo{
		parent:    parent,
		subFuncs:  []*funcInfo{},
...
func (f *funcInfo) addLocalVariable(name string, startPC int) int {
	newVar := &locVarInfo{
		name:    name,
...
func (f *funcInfo) slotOfLocalVariable(name string) int {
...
func (f *funcInfo) indexOfUpValue(name string) int {
	...
	if f.parent != nil {
		...
		if uvIdx := f.parent.indexOfUpValue(name); uvIdx >= 0 {
			idx := len(f.upvalues)
