...
func (l *Lexer) NextToken() (line, kind int, token string) {
	...
	case ',':
		l.next(1)
		return l.currentLine, TOKEN_SEP_COMMA, ","
	case '(':
		l.next(1)
		return l.currentLine, TOKEN_SEP_LPAREN, "("
	case ')':
		l.next(1)
		return l.currentLine, TOKEN_SEP_RPAREN, ")"
	case ']':
		l.next(1)
		return l.currentLine, TOKEN_SEP_RBRACK, "]"
	case '{':
		l.next(1)
		return l.currentLine, TOKEN_SEP_LCURLY, "{"
	case '}':
		l.next(1)
		return l.currentLine, TOKEN_SEP_RCURLY, "}"
	case '+':
		l.next(1)
		return l.currentLine, TOKEN_OP_ADD, "+"
	case '-':
		l.next(1)
		return l.currentLine, TOKEN_OP_MINUS, "-"
	case '*':
		l.next(1)
		return l.currentLine, TOKEN_OP_MUL, "*"
	case '^':
		l.next(1)
		return l.currentLine, TOKEN_OP_POW, "^"
	case '%':
		l.next(1)
		return l.currentLine, TOKEN_OP_MOD, "%"
	case '&':
		l.next(1)
		return l.currentLine, TOKEN_OP_BAND, "&"
	case '|':
		l.next(1)
		return l.currentLine, TOKEN_OP_BOR, "|"
	case '#':
		l.next(1)
		return l.currentLine, TOKEN_OP_LEN, "#"
	case ':':
		if l.test("::") {
			l.next(2)
			return l.currentLine, TOKEN_SEP_LABEL, "::"
		} else {
			l.next(1)
			return l.currentLine, TOKEN_SEP_COLON, ":"
		}
	case '/':
		if l.test("//") {
			l.next(2)
			return l.currentLine, TOKEN_OP_IDIV, "//"
		} else {
			l.next(1)
			return l.currentLine, TOKEN_OP_DIV, "/"
		}
	case '~':
		if l.test("~=") {
			l.next(2)
			return l.currentLine, TOKEN_OP_NE, "~="
		} else {
			l.next(1)
			return l.currentLine, TOKEN_OP_WAVE, "~"
		}
	case '=':
		if l.test("==") {
			l.next(2)
			return l.currentLine, TOKEN_OP_EQ, "=="
		} else {
			l.next(1)
			return l.currentLine, TOKEN_OP_ASSIGN, "="
		}
	case '<':
		if l.test("<<") {
			l.next(2)
			return l.currentLine, TOKEN_OP_SHL, "<<"
		} else if l.test("<=") {
			l.next(2)
			return l.currentLine, TOKEN_OP_LE, "<="
		} else {
			l.next(1)
			return l.currentLine, TOKEN_OP_LT, "<"
		}
	case '>':
		if l.test(">>") {
			l.next(2)
			return l.currentLine, TOKEN_OP_SHR, ">>"
		} else if l.test(">=") {
			l.next(2)
			return l.currentLine, TOKEN_OP_GE, ">="
		} else {
			l.next(1)
			return l.currentLine, TOKEN_OP_GT, ">"
		}
	case '.':
		if l.test("...") {
			l.next(3)
			return l.currentLine, TOKEN_VARARG, "..."
		} else if l.test("..") {
			l.next(2)
	...
	case '[':
		if l.test("[[") || l.test("[=") {
			return l.currentLine, TOKEN_STRING, l.scanLongString()
		} else {
			l.next(1)
			return l.currentLine, TOKEN_SEP_LBRACK, "["
		}
	case '\'', '"':
	...
	if c == '_' || isLetter(c) {
		...
		if kind, found := keywords[token]; found {
			return l.currentLine, kind, token // keyword
		} else {
			return l.currentLine, TOKEN_IDENTIFIER, token
		}
	}
...
func (l *Lexer) scanLongString() string {
	...
	l.currentLine += strings.Count(str, "\n")
	if len(str) > 0 && str[0] == '\n' {
		str = str[1:]
	}
...
func (l *Lexer) scanShortString() string {
	if str := reShortStr.FindString(l.srcCode); str != "" {
		...
		if strings.Index(str, `\`) >= 0 {
			l.currentLine += len(reNewLine.FindAllString(str, -1))
			str = l.escape(str)
		}
		return str
	}
